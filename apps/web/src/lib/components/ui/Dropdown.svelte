<script lang="ts">
	...
	import { ChevronSort } from 'carbon-icons-svelte';
...
>
	...
	{#if isOpen}
		<div
			class="inset-shadow absolute z-50 mt-1 flex min-w-full flex-col gap-1 rounded-sm bg-background-inset p-1"
			role="listbox"
			transition:popTransition
		>
			{#each items as item, i (item.value)}
				{@const itemClass = cn(
					'flex w-full items-start gap-3 rounded-xs px-3 py-1.5 text-left text-sm transition-colors',
					item.active
...
{#snippet dropdownItem(item: DropdownItem)}
	...
	<div class="min-w-0 flex-1">
		<span class="mb-0.5 line-clamp-1">{item.label}</span>
		{#if item.description}
